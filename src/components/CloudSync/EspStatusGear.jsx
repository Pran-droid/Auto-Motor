// EspStatusGear.jsx
// Uses the exact gear design provided by the user.
// Status is driven by className on the wrapper instead of :hover/:active,
// so the React prop controls the animation state:
//   idle/connecting → slow spin (4s, default)
...
const StyledWrapper = styled.div`
  ...
  .g1 { position: relative; left: 10px; }
  ...
  /* ── ERROR state: gears stop, turn red, pulse ── */
  .gear-wrapper.is-error .g1,
  .gear-wrapper.is-error .g2,
  .gear-wrapper.is-error .g3 {
    animation: none;
    transition: filter 0.4s;
    filter: drop-shadow(0 0 4px #c0392b);
  }
  .gear-wrapper.is-error {
    animation: ${errorPulse} 1.2s ease-in-out infinite;
  }
  .gear-wrapper.is-error svg path {
    fill: #c0392b;
...
const GEAR_PATH = `M5664 11538 c-38 -40 -55 -68 -68 -113 -19 -65 -44 -149 -70 -235 -9
...
function GearSvg({ className }) {
  return (
    <svg
      ...
      version="1.0"
    >
      <g stroke="none" fill="#067c40" transform="translate(0.000000,1280.000000) scale(0.100000,-0.100000)">
        <path d={GEAR_PATH} />
      </g>
    </svg>
  )
